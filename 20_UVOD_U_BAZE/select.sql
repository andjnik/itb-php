...
-- Cija je plata jednaka 500, 600 ilii 700
SELECT `name`, `address`, `state`, `salary`
FROM `customers`
WHERE `salary` = 500
OR `salary` = 600
...
-- (odredi korisnike sa vocifrenim brojem poseta, i prikazi prva dva sa
-- najvecim takvim brojem poseta)
SELECT *
FROM `customers`
WHERE `number_of_visits` BETWEEN 10 AND 99
ORDER BY `number_of_visits` DES
LIMIT 2;

-- Iz tabele customers, pročitati sve klijente koji su iz Srbije a imaju platu od 600:
SELECT *
FROM `customers`
WHERE `state` = 'Srbija'
AND `salary` = 600;
...
-- Odrditi broj razlicitih imena kupaca
